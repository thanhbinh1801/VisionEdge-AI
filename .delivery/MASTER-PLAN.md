...
status: in-review
updated_at: 2026-08-18T08:38:50+07:00
depends_on:
  - REQUIREMENTS.md
  - ARCHITECTURE.md
  - TECHNICAL-RISKS.md
...
Dự án được phân chia trách nhiệm theo **Phân hệ Nghiệp vụ (Feature Modules)** kết hợp phân công người phụ trách cụ thể (`Assigned to`) ở từng nhiệm vụ. Giai đoạn **Nền tảng dùng chung (Shared Core & Foundation)** sẽ được thực hiện tại Phase 1 để đảm bảo tính độc lập và khả năng phát triển song song tối đa giữa 2 Lập trình viên ngay từ ngày đầu tiên:

- **Giai đoạn Nền tảng Dùng chung (Shared Core & Foundation - Phase 1 Wave 1 & Wave 2)**:
  - `TASK-001` (Benchmark AI Model - Person A) và `TASK-005` (UI/UX Design System Contract - Person B) cùng được xếp tại Wave 1 của Phase 1, chạy song song tuyệt đối ngay từ ngày bắt đầu dự án mà không bị rào cản Phase Gate.
  - `TASK-002` & `TASK-003` (API Contracts, DB Schema & Seed Data): Person A phụ trách (chờ kết quả benchmark TASK-001 để chốt kiểu dữ liệu AI output).
  - `TASK-004` (Custom Label Embedding Architecture & ADR-005): Person B phụ trách (chờ TASK-001 để chốt backbone feature map).
  - `TASK-006` (Shared Database Access Layer): Person A phụ trách.
  - `TASK-007` (Zone Evaluator, Configurable Cooldown & Clip Slicer): Person B phụ trách.
  - `TASK-008` (Cross-Cutting Settings UI): Phân chia độc lập theo file: Person A làm Zone Editor UI (`zone_editor.js`), Person B làm Custom Labeler UI (`vehicle_tagger.js`, `custom_labeler.js`).
...
- **Phân hệ 3 — AI Assistant Q&A & Web Chat Integration (Phát triển song song tại Wave 1 Phase 2)**:
  - **Phân chia Task**: Tách thành `TASK-011a` (Backend Text-to-SQL Engine) và `TASK-011b` (Chatbot UI Frontend).
  - **Lưu ý Tiến độ**: Sử dụng dữ liệu seed/mock event sẵn có trong DB từ Phase 1 (`seed_events.sql`) để lập trình và test Text-to-SQL / Chat UI ngay tại Wave 1 của Phase 2 mà không phải chờ 2 module Cổng & Khu vực hoàn thành 100%.
...
## Phase 1 — AI Model Benchmarking, Shared Core, Foundations & Cross-Cutting Settings UI Design
...
- Parallel-safety notes: Chạy song song độc lập tại Wave 1 cùng TASK-005 (Person B). Video mẫu có sẵn làm đầu vào (input có trước, không phải deliverable của task này).
- Status: draft
...
- Write scope: docs/contracts/
- Wave: 1
- Completion gate: Định nghĩa hoàn chỉnh OpenAPI Spec (gồm endpoint `PATCH /api/events/{id}/correct-plate`) và JSON WebSocket Event Schemas.
- Verification method: Kiểm tra hợp lệ JSON Schema bằng `python -m json.tool docs/contracts/api-schema.json`.
- Parallel-safety notes: Cần giữ dependency với TASK-001 để chốt cấu trúc trường dữ liệu output từ AI Model (độ tin cậy OCR confidence score, enum loại đối tượng) vào API/WebSocket contract.
- Status: draft
...
- Outputs: `backend/db/schema.sql`, `backend/db/seed_events.sql`, `docs/contracts/db-contract.md`
- Write scope: backend/db/schema.sql, backend/db/seed_events.sql, docs/contracts/db-contract.md
- Wave: 1
- Completion gate: DDL đầy đủ CSDL SQLite (Camera, Zone, Vehicle, Event, CustomLabel, VideoClip) và bộ dữ liệu seed_events mẫu cho QA Agent.
- Verification method: Chạy `sqlite3 backend/db/test.db < backend/db/schema.sql` và nạp seed data thành công.
- Parallel-safety notes: Cần giữ dependency với TASK-001 để định nghĩa chính xác kiểu dữ liệu lưu trữ kết quả nhận diện biển số và nhãn đối tượng trong DDL SQLite.
- Status: draft
...
- Write scope: .delivery/
- Wave: 1
- Completion gate: Thống nhất và ban hành ADR-005 quy định cơ chế Few-shot Embedding Vector Matching (dùng Cosine Distance threshold) để phân loại mẫu custom label mà không cần fine-tune YOLO trong 2-4 tuần.
- Verification method: Kiểm tra sự tồn tại và nội dung của văn bản ADR-005.
- Parallel-safety notes: Cần giữ dependency với TASK-001 để biết cụ thể backbone/model được chọn (các lớp feature map của YOLOv8) nhằm thiết kế chính xác thuật toán trích xuất Feature Vector Embedding trong ADR-005.
- Status: draft
...
- Parallel-safety notes: Không phụ thuộc TASK-001. Chạy song song độc lập ngay tại Wave 1 của Phase 1 cùng TASK-001 (Person A) mà không bị rào cản Phase Gate.
- Status: draft
...
- Assigned to: Person A
- Required capability: backend-implementation
- Inputs: `backend/db/schema.sql`, `backend/db/seed_events.sql`
- Outputs: `backend/db/connection.py`, `backend/db/models.py`, `backend/db/crud.py`
- Write scope: backend/db/
- Wave: 2
- Completion gate: Khởi tạo ORM/CRUD kết nối SQLite, hàm query event, vehicle, zone và nạp seed dữ liệu thử nghiệm.
...
- Outputs: `backend/config.py`, `backend/ai/zone_evaluator.py`, `backend/events/cooldown_manager.py`, `backend/events/clip_slicer.py`
- Write scope: backend/config.py, backend/ai/zone_evaluator.py, backend/events/cooldown_manager.py, backend/events/clip_slicer.py
- Wave: 2
- Completion gate: Hàm Point-in-Polygon xử lý tâm BBox, Cooldown Engine dạng tham số cấu hình linh hoạt (đọc từ `config.py` / `.env`, giá trị khởi điểm 10-15s) và Clip Slicer 10s cứng theo RFP.
- Verification method: Chạy unit test `python -m pytest backend/tests/test_shared_utils.py` đạt 100% pass.
- Parallel-safety notes: Đóng gói tiện ích core dùng chung trước khi chia module.
- Status: draft
...
- Assigned to: Person A & Person B (Song song độc lập theo file)
...
- Wave: 2
- Completion gate: Vẽ zone đa giác SVG 2 chế độ Chọn/Vẽ kéo thả đỉnh mượt mà (Person A phụ trách `zone_editor.js`), gán nhãn xe quen/lạ 1-click và tool khoanh BBox custom dataset có timeline scrubber (Person B phụ trách `vehicle_tagger.js` & `custom_labeler.js`).
- Verification method: Mở trình duyệt kiểm tra thao tác vẽ zone và gán nhãn dataset.
- Parallel-safety notes: Phân chia rõ theo từng file riêng biệt không đụng nhau, cho phép 2 người triển khai song song tuyệt đối.
- Status: draft

---

## Phase 2 — Parallel Feature Modules Implementation & Integration Checkpoint
- Gate: integration-check
...
- Linked change requests: none
- Dependencies: TASK-006, TASK-007
- Assigned to: Person A/B (Linh hoạt - dành cho người hoàn thành Module Cổng/Khu vực trước)
- Required capability: backend-implementation
- Inputs: `docs/contracts/api-schema.json`, `backend/db/crud.py`, `backend/db/seed_events.sql`
- Outputs: `backend/api/qa_agent.py`, `backend/api/routes.py`, `backend/api/websocket_server.py`, `backend/main.py`
- Write scope: backend/api/qa_agent.py, backend/api/routes.py, backend/api/websocket_server.py, backend/main.py
- Wave: 1
- Completion gate: Xử lý câu hỏi tự nhiên tiếng Việt bằng Text-to-SQL trên CSDL events (dùng seed data hoặc event thật), trả về câu trả lời tự nhiên đính kèm link clip 10s.
...
- Assigned to: Person A/B (Linh hoạt - dành cho người còn lại)
- Required capability: frontend-implementation
- Inputs: `docs/contracts/api-schema.json`, `docs/contracts/websocket-events.json`, `Prototype/Intern-LPR-Gate.dc.html`
- Outputs: `frontend/src/chat/ai_chatbot.js`, `frontend/src/chat/video_modal.js`, `frontend/src/app.js`
- Write scope: frontend/src/chat/, frontend/src/app.js
- Wave: 1
- Completion gate: Giao diện Chatbot AI đính kèm trình phát video MP4 10s (có nút tải clip) và Prompt Chips gợi ý (dùng mock response dựa trên API contract TASK-002 trước khi API thật sẵn sàng).
...
## Phase 3 — End-to-End System Verification & Project Deliverables Packaging
- Gate: user-approval
...
- Outputs: `README.md`, `docker-compose.yml`, `docs/deployment-guide.md`
- Write scope: README.md, docker-compose.yml, docs/deployment-guide.md
- Wave: 1
- Completion gate: Xây dựng tài liệu hướng dẫn cài đặt, cấu hình môi trường, chạy ứng dụng bằng lệnh local/docker-compose theo đúng Mục 4 của RFP bài tập.
...
- Write scope: docs/reports/
- Wave: 1
- Completion gate: Kiểm thử E2E toàn bộ 8 Yêu cầu bắt buộc của RFP trên cả 4 tab giao diện, xác nhận FPS >= 5 trên 2 video stream, trích xuất clip 10s quanh thời điểm sự kiện, fallback sửa tay biển số OCR và hỏi đáp AI Chatbot chính xác (tiêu chí độ trễ < 1s là tiêu chí chất lượng tự bổ sung ngoài RFP, không bắt buộc).
