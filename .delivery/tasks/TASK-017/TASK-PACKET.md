...
- Completion gate: Frame loop area monitoring does not read DB per frame; zone rules are served from in-memory cache by `camera_id`; metadata realtime and event persistence lanes remain separated.
