...
# Task Result: TASK-012 — Đồng bộ Zone Editor sang Area Dashboard

- Task ID: TASK-012
- Outcome: blocked
- Inputs used: TASK-012 packet/diagnosis và approved inputs đọc từ `HEAD`; backend frame/pipeline và frontend TASK-010/TASK-012 hiện tại.
- Outputs produced: Area Dashboard hiển thị tức thì geometry/name của zone vừa chỉnh từ shared state; MJPEG hỗ trợ tắt backend zone rendering để tránh polygon trùng; backend tiếp tục là bbox renderer duy nhất.
- Validation evidence: Backend compile và scoped Ruff pass; toàn bộ backend 31 passed; frontend lint và TypeScript pass; production Vite build pass với 38 modules transformed; scoped diff check pass.
...
- Deviations: Không thay đổi response body contract. `/events/video-feed` chỉ nhận thêm optional `draw_zones` mặc định `true`, nên client cũ giữ nguyên hành vi.
