...
- Acceptance criteria: 1. Mô hình YOLOv26 xác định chính xác bounding box và vị trí tâm đối tượng nằm trong hay ngoài đa giác zone (Point-in-polygon). 2. Phát cảnh báo ngay lập tức nếu loại đối tượng nằm trong danh sách cấm của zone. 3. Hiển thị danh sách sự kiện khu vực trên React Event Feed với trạng thái rõ ràng (Được phép, Vi phạm). 4. Hiển thị đầy đủ bộ 4 thẻ KPI thống kê realtime cho khu vực bãi kiểm qua Recharts visualizers.
- Status: approved
- Delivery classification: changed
...
- Behavior: Áp dụng cơ chế cửa sổ thời gian (Cooldown 10-15 giây) cho mỗi đối tượng/biển số trong cùng một Zone. Khi một đối tượng đứng yên hoặc di chuyển trong zone liên tục, hệ thống chỉ sinh 1 Event chính và 1 Clip 10s đại diện. React UI lắng nghe WebSocket event deduplicated để cập nhật feed mà không làm giật lag giao diện.
...
- Acceptance criteria: 1. Trong vòng 15s, cùng một biển số/đối tượng lưu lại trong zone chỉ tạo đúng 1 bản ghi sự kiện. 2. Nếu đối tượng rời zone và quay lại sau khoảng thời gian cooldown, sự kiện mới được khởi tạo bình thường và hiển thị lên React UI Feed.
- Status: approved
- Delivery classification: changed
...
- Acceptance criteria: 1. Cấu hình zone vẽ trên React SVG Canvas UI được cập nhật ngay lập tức xuống AI pipeline mà không cần restart server. 2. Hỗ trợ thao tác kéo thả đỉnh, thêm đỉnh ở cạnh, di chuyển thân zone mượt mà trên React SVG Canvas với Custom Hook `usePolygonEditor`. 3. Cập nhật bảng quy tắc cấm/cho phép theo từng loại xe/đối tượng ngay trên thẻ điều khiển zone với Lucide React icons (Check, X).
- Status: approved
- Delivery classification: changed
...
- Behavior: Khi phát sinh sự kiện Mức 3 (Vi phạm zone cấm / Xe cấm xâm nhập), hệ thống phát âm thanh cảnh báo còi hiệu thời gian thực thông qua React shared component `<AudioBeepPlayer>` (Web Audio API context) và popup nổi bật trên React UI, đồng thời gửi thông báo qua Telegram Bot / Zalo OA.
...
- Acceptance criteria: 1. Component `<AudioBeepPlayer>` phát tiếng bíp còi hiệu ngay trên trình duyệt khi có WebSocket event Mức 3. 2. Gửi tin nhắn chứa chi tiết thời gian, camera, zone vi phạm và ảnh crop sang Telegram Bot trong thời gian < 2 giây.
- Status: approved
- Delivery classification: changed
